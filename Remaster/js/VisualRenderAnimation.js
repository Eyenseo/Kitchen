 ...
 *		  	};
 */
function VisualRenderAnimation(context, data) {
	VisualRenderObject.call(this, context, data.sx, data.sy, data.w, data.h, data.picture, data.zOrder);

	this.animations = data.aniObject.animations;

	this.tileWidth = data.aniObject.image.tileWidth || data.w;
	this.tileHeight = data.aniObject.image.tileHeight || data.h;

	// needed for the calculation of the x, y coordinate for one tile
	this.maxCols = data.aniObject.image.imgWidth / this.tileWidth || 1;
	// Not needed ...
...
VisualRenderAnimation.prototype.changeAnimation = function(name, keepProgress) {
	try {
		...
		throw "There is no animation named:" + name;
	}
};
